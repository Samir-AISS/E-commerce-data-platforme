...
# ── DB ────────────────────────────────────────────────────────────────────────
DB_URL = os.getenv("DATABASE_URL", "postgresql://ecommerce:ecommerce123@postgres/ecommerce_db")
...
# ════════════════════════════════════════════════════════════════════════
    ...
    df = query("""
        SELECT order_date, order_year, order_month_num, order_dow,
               n_orders, gross_revenue, avg_order_value,
               delivered_orders, cancel_rate_pct, late_delivery_pct
        FROM bronze_gold.gold_revenue_daily
        ORDER BY order_date
    """)
    ...
    if not df.empty:
        ...
        with cl:
            ...
            df_dow["day"] = df_dow["order_dow"].map(lambda x: dow_labels[int(x)])
            fig = go.Figure(go.Bar(
...
# ════════════════════════════════════════════════════════════════════════
    ...
    df_rfm = query("""
        ...
        FROM bronze_gold.gold_customer_rfm
    ...
    if not df_rfm.empty:
        ...
        with c2: st.markdown(kpi("Champions",
            f"{df_rfm[df_rfm.rfm_segment=='Champions']['n'].sum():,}",
            "top segment", "pos"), unsafe_allow_html=True)
        with c3: st.markdown(kpi("At Risk",
            f"{df_rfm[df_rfm.rfm_segment=='At Risk']['n'].sum():,}",
            "need attention", "neg"), unsafe_allow_html=True)
    ...
    df_state = query("""
        ...
        FROM bronze_silver.silver_customers
...
# ════════════════════════════════════════════════════════════════════════
    ...
    df_cat = query("""
        ...
        FROM bronze_gold.gold_product_performance
    ...
    if not df_cat.empty:
        ...
        with c3: st.markdown(kpi("Best Rated", df_cat.loc[df_cat.avg_review.idxmax(),"category_english"], f"{df_cat.avg_review.max():.2f}/5", "pos"), unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════
    ...
    df_rev = query("""
        ...
        FROM bronze_bronze.bronze_reviews
    ...
    if not df_rev.empty:
        ...
        with cl:
            ...
            fig = go.Figure(go.Bar(
                x=df_rev["review_score"].astype(str),
                y=df_rev["n"],
        ...
        df_cat_rev = query("""
            ...
            FROM bronze_bronze.bronze_reviews r
            JOIN bronze_silver.silver_orders o USING(order_id)
            JOIN bronze_bronze.bronze_order_items oi USING(order_id)
            JOIN bronze_bronze.bronze_products p USING(product_id)
        ...
        if not df_cat_rev.empty:
            fig = go.Figure(go.Bar(
                x=df_cat_rev["avg_score"],
                y=df_cat_rev["category_english"],
                orientation="h",
